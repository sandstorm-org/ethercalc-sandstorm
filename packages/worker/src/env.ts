...
export interface Env {
  ...
  /**
   * Single-grain default room or workbook. When set, `GET /`
   * 302-redirects into `/<ETHERCALC_DEFAULT_ROOM>` instead of serving
   * the "create new sheet" landing page. Used by Sandstorm grains to
   * take the user straight to the workbook on `/`. Unset on ethercalc.net
   * (shared pool of rooms, where `/` legitimately is a landing page).
   */
