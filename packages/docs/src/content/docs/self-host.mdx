...
Each Sandstorm grain runs one workbook (`ETHERCALC_DEFAULT_ROOM="=sheet"`). The workbook TOC lives in room `sheet`, and the first visible sheet is `sheet1`. Packaging files (`run_grain.sh`, `sandstorm-pkgdef.capnp`) and the `.spk` build recipe are in [SANDSTORM.md](https://github.com/audreyt/ethercalc/blob/main/SANDSTORM.md). Legacy grains auto-migrate `/var/dump*` into `sheet1` on first boot.
